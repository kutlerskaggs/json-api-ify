        ...
        async.auto({
            validate: function validatePayload(fn) {
                let schema = joi.object({
                    meta: joi.object(),
                    links: joi.object(),
                    included: joi.array().items(joi.object()),
            ...
            deserializeData: ['validate', function deserializeData(fn) {
            ...
            deserializeIncluded: ['validate', function deserializeIncluded(fn) {

                if (internal.options.deserializeIncludedRelationships) {
                    async.eachSeries(payload.included, function (resource, _fn) {
                        util.deserializeResource(internal, resource, data, _fn);
                    }, fn);
                } else {
                    fn();
            ...
            processRelationships: ['deserializeData', 'deserializeIncluded', function processRelationships(fn) {
                let visited = [];

                async.eachSeries(payload.data, function(resource, _fn) {
                    util.processRelationships(internal, resource, payload.included || [], visited, data, _fn);
                }, fn);
            }]
        }, function(err) {
            if (err) {
        ...
        self._serialize(type, schemaName, data, options, function(err, payload) {
            ...
            */
            ...
            if (internal.options.minimizePayload === true || options.minimizePayload === true) {
                payload = util.minimizePayload(payload);
            }
